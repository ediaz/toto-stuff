...
oz,dz,nz = 0.0,0.00762*3,401
ox,dx,nx = 0.0,0.00762*3,1067
ot,nt,dt =    0.0, 6001, .002
fpeak = 9.0
ns = 1
###########################################################################
def main(args):
...
def goSigsbeeShot():
  ...
  nr = 348
  rvr_coo = zerofloat(2,nr) 

  for ir in range(nr):
    rvr_coo[ir][0] = xs + ir*dx
    rvr_coo[ir][1] = 2*dz

  fdobj = fdmod.Fdmod2d(ox,dx,nx,oz,dz,nz,ot,dt,nt,mvel,43,False) 
  ...
  sw.start()
  fdobj.forwardAll()
  print "modeling took %10.5g sec"%(sw.time())
  dat = fdobj.getData()
  plot(dat,"shot")
