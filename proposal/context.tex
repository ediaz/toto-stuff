...
the image point $\xx$ is neglected, \rfg{BornImaging}. Alternatively, if we take into account
all the possible paths that connect the source $\xx_s$ with the receiver $\xx_r$ through the image point $\xx$,
 we can illuminate this point from many different angles, \rfg{MultiImaging}. Illumination from 
...
published on the subject \citep{guittonTLE} \red{(this is the introduction article for the TLE 2013 FWI issue)}. The goal of FWI 
...
\cite{GPR:GPR698} proposes a least-squares migration scheme for an extended perturbation, which
...
stochastic optimization approaches, which might be able to  
 find models that can fill the gap present in the seismic data (\rfg{Claerbout}). An additional benefit
