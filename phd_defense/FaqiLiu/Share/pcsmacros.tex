...
\def\tbox#1#2{
\linethickness{1mm}
  \put(0,0){\put(0,0.5){\line(1,0){#1}}}
  \put(#1,0){\line(0,1){#2}}
  \put(#1,#2){\put(0,-0.5){\line(-1,0){#1}}}
  \put(0,#2){\line(0,-1){#2}}
}

\def\ybox#1#2#3#4{
   \yellow{\put(#1,#2){\tbox{#3}{#4}}}
}
...
% Norm symbol
\newcommand{\norm}[2]{\left|\left|#1\right|\right|^{2}_{#2}}
\newcommand{\ltnorm}[1]{\norm{#1}{2}}
...
\def\swe#1{ \frac{1}{v^2}\dtwo{#1}{t}- \LAPL{#1}}
...
\def\glabel#1#2#3{ \darkgreen{ \biglabel{#1}{#2}{#3} }}
