...
class LocalNormCorr:
  ...
  def stateVariables(self,m,feval=True):
    ...
    for iexp in range(self.nexp):
      ...
      Flow(mdat,mdatT,'costaper nw1=100 |transp')
  ...
  def of(self,OF,m,feval=True):
    ...
    self.nof  = self.OF+'-neg'
    self.ndofd = self.ofd+'-dneg'
  
    Flow(self.nof,self.OF,'scale rscale=-1')
    Flow(self.ndofd,self.ofd,'math output="-1/input"')
  ...
  def gradient(self,GR,OF,m):
    ...
    'math m=${SOURCES[1]} output="-2e8/m^3*input"|costaper nw2=70|scale axis=123|'+self.prec)
